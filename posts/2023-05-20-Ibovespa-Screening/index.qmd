...
Although this is totally up to you, I fully recommend using the [`pacman`](https://cran.r-project.org/web/packages/pacman/pacman.pdf) package (`install.packages('pacman')`) for package management. More specifically, we'll provide a vector of strings with all libraries, and `p_load()` takes care of the rest: if you have a given package already installed, `p_load()` will load it into your R session; otherwise, the package will be first installed, and then loaded.
...
packages.to.install=c("ggplot2","dplyr","tidyr","PerformanceAnalytics","quantmod","xts","ggpubr","zoo","alphavantager","quantmod","flextable")

sapply(packages.to.install,library,character.only=TRUE,quietly=TRUE)
